...
**The pawn economy:** three currencies -- **MEAT, GOLD, WOOD** -- fill each faction's war chest (free villages share one Villager chest, as if trading). Peon deliveries bank 1 of their job's resource: herders bring meat, miners gold, cutters wood. Capitals are **mixed economies** with all three worked in arcs; the countryside is single-theme, so the map writes your income. Production is people: **houses raise recruit pawns** (1 meat each) who walk to a production building in their settlement and **convert into its soldier** at the door -- gold arms warriors and monks, wood arms archers and lancers. Recruits are visible on the roads: queues form outside barracks when a chest runs dry, and a battle at the gates keeps them fleeing instead of enlisting -- sieges choke production with no extra rules. Reinforcement flow scales with houses held and arrives **distributed across the empire**; the army cap (walking recruits included) is the throttle. Neutral settlements run the same rule in Black: their converts stay home as tier-capped militia (defend-zone stance) and pledge to a captor when the village falls. Peons **flee combat** and are never targeted. Militia of quiet settlements **sleeps** until a hostile enters the influence radius. A **landless army starves**: after a short grace its units wither and its monks have nothing to heal with. Soldiers of your color stationed at settlements are ordinary recruits for your retinue; stationed monks patrol their zone healing any wounded friendly.
...
Reinforcements are raised locally by the pawn economy (see above): houses spawn recruits, recruits enlist at buildings, and the army's composition reflects the resources held -- gold territory fields warriors and monks, wood territory archers and lancers, and everyone needs pastures to grow at all. Losing ground literally starves the war effort; taking a settlement seizes its houses and income.
...
- **HUD** -- player health, authority rank, retinue counter (current/cap), the **resource tray** (meat/gold/wood icons + counts for your faction), a settlement strip whose pips shrink to fit any settlement count, and one war-score counter per active faction in its color. Settlement banners carry their resource-theme icon (capitals show the trio); production buildings raise plates when a queue stalls (the missing resource, blinking) or when the unaligned player stands near (the unit you would become); deliveries and conversions pop tinted +1/−1 ticks in the world.
