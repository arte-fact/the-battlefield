...
pub struct TerrainConfig {
    /// Number of hill clusters (elevated terrain).
...
pub fn generate_battlefield_with_config(seed: u32, config: &TerrainConfig) -> Grid {
    ...
    // Place hill clusters as elevated grass in the contested middle area
    for _ in 0..config.hill_clusters {
        ...
        place_elevation_cluster(&mut grid, &mut rng, cx, cy, radius, 2, 0.7);
    }
...
/// Place a roughly circular cluster of elevated grass tiles (hills).
fn place_elevation_cluster(
    grid: &mut Grid,
    rng: &mut Rng,
    cx: u32,
    cy: u32,
    ...
    for dy in -r..=r {
        for dx in -r..=r {
            ...
            let dist_sq = dx * dx + dy * dy;
            if dist_sq <= r * r && rng.chance(density) {
                grid.set_elevation(nx as u32, ny as u32, elevation);
            }
        }
    }
}
...
/// Clear a rectangular zone back to grass with zero elevation (for deployment areas).
